...
void main()
{
    ...
    vMat.view=view;
    vMat.proj=proj;
    vMat.NormalMatrix = transpose(inverse(mat3(vMat.view*vMat.model)));
    mat3 tv = transpose(mat3(vMat.view));
    vOutput.Normal = normalize(vMat.NormalMatrix*normal);
    vOutput.Tangent = normalize(vMat.NormalMatrix*tv[0]);
    vOutput.BiTangent = normalize(vMat.NormalMatrix*tv[1]);
    vMat.tbn = transpose(mat3(vOutput.Tangent,vOutput.BiTangent,vOutput.Normal));
...
vec4 getLightsStupid()
{
    ...
    {
        vec3 LightToVertex = vec3((vMat.view*vec4(LightPositions[i],1)).xyz-vOutput.cameraPosition.xyz);   //Surface to light
        float Distance = length(LightToVertex);
        LightToVertex = normalize(LightToVertex);
        vec3 R = reflect(-LightToVertex,n);
        vec3 v = normalize((-vOutput.cameraPosition.xyz));
...
{
    vec3 V = normalize(tbnMatrix*(-vOutput.cameraPosition.xyz));
...
vec4 getLights(vec2 texcoord, sampler2D norm,mat3 tbnMatrix){
    ...
    vec3 bump = normalize(texture2D(norm, texcoord).xyz*2-1);    //unit vector in direction of the texture bump
    vec4 specularMap = texture2D(Texture_Specular0, texcoord);
    vec4 FINALLIGHT = GlobalAmbient*ObjectAmbient;
    for(int i=0;i<lightCount&&i<lightMax;++i){

        vec3 l = tbnMatrix*((vMat.view*vec4(LightPositions[i],1)).xyz-vOutput.cameraPosition.xyz);   //vector with direction of light to vertex
        vec3 v = tbnMatrix*(-vOutput.cameraPosition.xyz);
...
void main() {
    ...
    vec2 DisplacedTexCoord = vOutput.Texcoord;
    ...
    if(TextureOn)
    {
        if(isCube)
        {
            outColor *= vec4(texture(CubeTexture,vOutput.modelPosition.xyz).xyz,1.f);
        }
        else
        {
            outColor *= vec4(texture(Texture_Diffuse0,DisplacedTexCoord).xyz,1.f);
        }
    }
    //if(ColorOn)
}
