...
void PhysicsObject::SetCollisionShape(btCollisionShape* col)
{
    CollisionShape = col;
}
void PhysicsObject::SetCollisionShape(Mesh* collShape, bool replaceDuplicates)
{
...
void PhysicsObject::ConstructRigidBody(btScalar mass, glm::vec3 Intertia)
{
...
void PhysicsObject::ConstructRigidBody(btRigidBody::btRigidBodyConstructionInfo CI)
{
    RigidBody = new btRigidBody(CI);
}
void PhysicsObject::AddToWorld(btDiscreteDynamicsWorld *addto)
{
    if(RigidBody!=NULL)
    {
        world = addto;
...
bool PhysicsObject::isCollidedWith(PhysicsObject *Other)
{
...
void PhysicsObject::setLinearVelocity(glm::vec3 vel)
{
    if(RigidBody!=NULL)
        RigidBody->setLinearVelocity(btVector3(vel.x,vel.y,vel.z));
}
void PhysicsObject::setAngularVelocity(glm::vec3 vel)
{
   // if(RigidBody!=NULL)
